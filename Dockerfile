...
FROM busybox:stable
ENV APP_HOME /app
RUN wget -O /usr/local/bin/curl https://github.com/moparisthebest/static-curl/releases/download/v7.83.0/curl-amd64 && chmod 755 /usr/local/bin/curl && adduser 1001 -D -h $APP_HOME && mkdir -p $APP_HOME && chown 1001:1001 $APP_HOME
USER 1001
WORKDIR $APP_HOME
